...
export default class Settings extends HTMLElement {
	...
	{
		let statistics = await lang.stats.statistics[getCookie("lang")];
		let history = await lang.stats.history[getCookie("lang")];
		let winnings = await lang.stats.winnings[getCookie("lang")];
		let looses = await lang.stats.looses[getCookie("lang")];
		let game = await lang.stats.game[getCookie("lang")];
	...
	}).then(json =>
	{
		...
		function GetHistory() {
			...
			const uri = `https://${location.hostname}:4343/player?player_id=${id}`;
			fetch(uri, {
				method: "GET",
				headers: header,
				body: null,
		...
		function GetStats() {
			...
			fetch(`https://${location.hostname}:5151/stats/${id}/infos`, {
				method: "GET",
				headers: header,
				body: null,
			})
