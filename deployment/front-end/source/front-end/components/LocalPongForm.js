...
export default class LocalPongForm extends HTMLElement {
    constructor() {
        super();
    }

    async connectedCallback() {
        let local_pong = await lang.local_games.local_pong[getCookie("lang")];
        ...
        this.innerHTML = `
            <div id="local-pong">
                <h1 id="title">${local_pong}</h1>
                <h2 id="form-title">${players_names}</h2>
                
                <div id="warning"></div>
    
                <div>
                    <form id="local-pong-form" class="form">
                        <div>
                        ...
                        <div id="button-div">
                            <button type="submit" id="submit-local-pong" class="submit-button">${start}</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        const submitButton = this.querySelector('#submit-local-pong');
        ...
        submitButton.addEventListener('click', e => {
            ...
            changeRoute(encodeURI(`/pong/local/game?player1=${player1Input.value}&player2=${player2Input.value}`));
        });
    }
}
